...
export default function Expertises() {
  return (
    <section className="py-32 px-6 md:px-12 max-w-7xl mx-auto w-full relative z-10" id="expertises">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {expertises.map((exp) => (
          <div key={exp.id} className="relative group p-[1px] rounded-3xl overflow-hidden min-h-[340px]">
            ...
            <div className="glass-panel p-10 relative h-full w-full rounded-[23px] flex flex-col justify-between overflow-hidden transition-all duration-500 group-hover:-translate-y-1 bg-[#050505]/60">
              ...
              <div className="relative z-10">
                <div className="mb-10 p-4 bg-white/5 border border-white/10 backdrop-blur-xl w-fit rounded-2xl group-hover:bg-accent/10 group-hover:border-accent/30 transition-all duration-500 shadow-[0_4px_30px_rgba(0,0,0,0.5)]">
                  {getIcon(exp.id)}
                </div>
                <h3 className="text-2xl font-bold text-primary mb-4 tracking-tight drop-shadow-md">{exp.title}</h3>
                <p className="text-secondary leading-relaxed text-base font-light">{exp.description}</p>
              </div>
            </div>
          </div>
        ))}
